<script lang="ts">
	...
	import type { OnboardingViewModel } from '../onboarding.svelte.js';
	import * as m from '$lib/paraglide/messages.js';

	let { vm }: { vm: OnboardingViewModel } = $props();
	...
	// After OAuth return, if session exists — check for returning user
	$effect(() => {
		if (isLoggedIn) {
			vm.checkReturningUser();
		}
	});
...
<div class="slide-layout">
	...
	<div class="slide-bottom pb-12">
		...
		>
			{#if vm.loading}
				<Icon name="loader-circle" size={20} class="animate-spin" />
			{:else}
